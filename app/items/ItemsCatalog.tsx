...
export function ItemsCatalog({
  ...
  useEffect(() => {
    ...
    if (!hasHandledInitialRender.current) {
      ...
      if (hasInitialData && currentBranchKey === initialBranchKey) {
        setIsLoading(false)
        return
      }
    }
  ...
  const filtered = items.filter(i => {
    ...
    const matchTab =
      activeTab === 'all' ? true
      : activeTab === 'low' ? isInventoryItem && isLowStock(i.quantity, i.reorderLevel)
  ...
  const lowStock = inventoryItems.filter(i => isLowStock(i.quantity, i.reorderLevel))
  ...
  function stockBadge(item: Item) {
    ...
    if (isLowStock(item.quantity, item.reorderLevel)) return <Badge variant="amber">Low</Badge>
            ...
            <div className="hidden md:block bg-white shadow-sm ring-1 ring-black/5 overflow-hidden">
              <table className="w-full">
                ...
                <tfoot className="bg-gray-50/80 border-t border-gray-100">
                  <tr>
                    <td colSpan={isSelectMode ? 8 : 7} className="px-5 py-3 text-xs font-bold text-gray-600">
