        ...
        <div className="relative -mt-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-5">
          ...
          {loading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="bg-white rounded-2xl shadow-sm ring-1 ring-black/5 p-4 h-24 animate-pulse" />
              ))}
            </div>
          ) : data && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          {!loading && data && (
            <>
              <div className="hidden sm:flex gap-1 bg-white rounded-2xl shadow-sm ring-1 ring-black/5 p-1.5">
                {TABS.map(t => (
                  <button
                    key={t.id}
                    onClick={() => setTab(t.id)}
...
function OverviewTab({ data }: { data: DashboardData }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
      {/* Agents summary */}
      <section className="bg-white rounded-2xl shadow-sm ring-1 ring-black/5 p-5">
        ...
        {data.agentSummary.pending > 0 && (
          <Link
            ...
            className="mt-4 flex items-center gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5 hover:bg-amber-100 transition-colors"
          >
...
function ApplicationsTab({
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="bg-white rounded-2xl shadow-sm ring-1 ring-black/5 overflow-hidden">
            ...
            {agents.map(agent => (
              <li key={agent.id}>
                <Link
                  ...
                  className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors"
                >
                  <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center shrink-0 font-bold text-xs text-slate-600 mt-0.5">
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-2xl shadow-2xl md:hidden max-h-[80vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-gray-100 shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-indigo-700 flex items-center justify-center text-white font-bold text-sm shrink-0">
                  {initials}
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {group.links.map(({ name: label, href, Icon }) => (
                      <Link
                        ...
                        href={href}
                        onClick={() => setDrawerOpen(false)}
                        className="flex flex-col items-center gap-2 py-3 px-2 rounded-xl bg-gray-50 text-gray-600 hover:bg-gray-100 text-center transition-colors"
                      >
                        <Icon className="w-5 h-5" strokeWidth={1.75} />
...
function KpiCard({
  ...
  return (
    <Link href={href} className="bg-white rounded-2xl shadow-sm ring-1 ring-black/5 p-4 hover:shadow-md transition-shadow block">
...
function MiniStat({
  ...
  return (
    <div className={`rounded-xl px-3 py-2.5 ${alert ? 'bg-amber-50 border border-amber-200' : 'bg-gray-50'}`}>
