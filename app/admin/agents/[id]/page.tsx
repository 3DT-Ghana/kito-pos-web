  ...
  return (
    <AdminLayout>
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {result && (
          <div className={`text-sm px-4 py-3 border ${result.type === 'success' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-red-50 text-red-700 border-red-100'}`}>
            {result.text}
          </div>
        )}
        ...
        <div className="flex gap-2 flex-wrap">
          {agent.status === 'PENDING' && (
            <>
              <button
                ...
                className="px-4 py-2 bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 disabled:opacity-50 transition-colors"
              >
                Approve Agent
              </button>
              <button
                ...
                disabled={actionLoading}
                className="px-4 py-2 bg-red-50 text-red-700 border border-red-200 text-sm font-semibold hover:bg-red-100 disabled:opacity-50 transition-colors"
              >
                Reject
              </button>
            </>
          ...
          {agent.status === 'APPROVED' && (
            <button
              ...
              disabled={actionLoading}
              className="px-4 py-2 bg-gray-100 text-gray-700 border border-gray-200 text-sm font-semibold hover:bg-gray-200 disabled:opacity-50 transition-colors"
            >
              Suspend Agent
            </button>
          )}
          {agent.status === 'SUSPENDED' && (
            <button
              onClick={() => updateStatus('APPROVED')}
              disabled={actionLoading}
              className="px-4 py-2 bg-emerald-50 text-emerald-700 border border-emerald-200 text-sm font-semibold hover:bg-emerald-100 disabled:opacity-50 transition-colors"
            >
              Reinstate Agent
            </button>
          )}
          <Link
            href="/admin/reports"
            className="px-4 py-2 bg-indigo-50 text-indigo-700 border border-indigo-200 text-sm font-semibold hover:bg-indigo-100 transition-colors"
          >
            Full Reports
          </Link>
        </div>
        ...
        {agent.status === 'PENDING' && (
          <div className="bg-white border border-gray-200 p-5">
            ...
            <input
              ...
              className="w-full max-w-lg px-3 py-2 border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
        )}
