      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <input
            ...
            className="w-full sm:w-56 px-4 py-2 border-2 border-gray-200 rounded-xl text-sm focus:border-indigo-500 focus:outline-none"
          />
        </div>
                      ...
                      <tr key={agent.id} className="hover:bg-gray-50 transition-colors group">
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-lg bg-linear-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs shrink-0 shadow-sm">
                          ...
                          <div className="flex items-center gap-2 justify-end">
                            {agent.status === 'PENDING' && (
                              <>
                                ...
                                <button
                                  ...
                                  disabled={actionId === agent.id}
                                  className="text-xs px-3 py-1.5 bg-red-50 text-red-700 border border-red-200 rounded-lg hover:bg-red-100 disabled:opacity-50 font-semibold transition-colors"
                                >
                                  Reject
                                </button>
                              </>
                            ...
                            {agent.status === 'APPROVED' && (
                              <button
                                ...
                                disabled={actionId === agent.id}
                                className="text-xs px-3 py-1.5 bg-gray-100 text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-200 disabled:opacity-50 font-semibold transition-colors"
                              >
                                Suspend
                              </button>
                            )}
                            {agent.status === 'SUSPENDED' && (
                              <button
                                onClick={() => updateStatus(agent.id, 'APPROVED')}
                                disabled={actionId === agent.id}
                                className="text-xs px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg hover:bg-emerald-100 disabled:opacity-50 font-semibold transition-colors"
                              >
                                Reinstate
                              </button>
                            )}
                            <Link
                              href={`/admin/agents/${agent.id}`}
                              className="text-xs px-3 py-1.5 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-lg hover:bg-indigo-100 font-semibold transition-colors"
                            >
                              View
                            </Link>
                          </div>
...
function ScoreCard({
  ...
  return (
    <div className={`bg-white rounded-xl border border-gray-200 shadow-sm p-4 ${alert ? c.ring : ''}`}>
